...
app.get('/api/:entity', (req, res) => {
    ...
    const entity = (currentConfig.entities || []).find((item) => 
        item.name.toLowerCase() === entityKey.toLowerCase() || 
        item.name.toLowerCase() === entityName.toLowerCase()
    );
    if (!entity) {
    ...
    const rows = Array.from({ length: 5 }, (_, i) => {
        ...
        (entity.fields || []).forEach((field) => {
            record[field.name] = field.type === 'uuid' ? `${entityKey}-${i + 1}` : 
                                 field.type === 'integer' ? i + 1 : 
                                 field.type === 'boolean' ? i % 2 === 0 : 
                                 field.options ? field.options[i % field.options.length] : 
                                 `${field.name}-${i + 1}`;
        });
        return record;
    });
